...
function createPresignedPutUrl(config: NonNullable<ReturnType<typeof getR2Config>>, key: string, contentType: string, expiresIn = 3600) {
  ...
  return { url: finalUrl, key, expiresAt: Math.floor(Date.now() / 1000) + expiresIn }
}
...
export async function POST(req: Request) {
  try {
    ...
    const result = createPresignedPutUrl(config, key, "image/png")
    if (!result) {
