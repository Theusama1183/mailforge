...
export async function POST(req: Request) {
  try {
    ...
    const { error } = await supabase.from("user_sessions").upsert(payload, { onConflict: "session_id", ignoreDuplicates: false })

    if (error) {
      const admin = createAdminClient()
      await admin.from("user_sessions").upsert(payload, { onConflict: "session_id", ignoreDuplicates: false })
    }
